...
#' @param wait A logical indicating whether to wait for the model to finish before proceeding.  The default is FALSE, to run models asynchronously.
...
runflow <- function(deckname,
  ...
  if(!dir.exists(file.path(basedir, "OUTPUT")) ||
     ...
     !dir.exists(file.path(basedir, "REPORTS"))){makeproj(deckname, basedir)}
