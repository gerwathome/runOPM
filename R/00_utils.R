...
rss2d <- function(design, lower, upper, gof.test.type="greenwood", gof.test.stat=NULL, transform=NULL, n.angle=360, graphics=1, trace=TRUE, lines.lwd=1, lines.lty="dotted", ...)  {
  ...
  for (i in 1:(d-1)) {
    ...
    for (j in ((i+1):d)) {
      ...
      out <- .C("C_rss2Dloop", as.double(x), as.double(y), as.double(cos.theta), as.double(sistrwidthn.theta), as.integer(n), as.integer(n.theta), ans=double(n * n.theta), PACKAGE="runOPM")
...
iwanthue <- function(n, hmin=0, hmax=360, cmin=0, cmax=180, lmin=0, lmax=100,
  ...
  clus <- stats::kmeans(coords(lab), n, iter.max = 50)
  if (isTRUE(plot)) {
...
#------------------------------------------------------------------------------
swatch <- function(x) {
  graphics::par(mai = c(0.2, max(graphics::strwidth(x, "inch")
                                 + 0.4, na.rm = TRUE), 0.2, 0.4))
  graphics::barplot(rep(1, length(x)), col = rev(x), space = 0.1, axes = FALSE,
