...
test_that("RunFlow works", {
  expect_warning(RunFlow("not_here.data", "testsim"),"Failed to find ")
  expect_error(RunFlow(deck1, "testsim"),NA)
  expect_equal(list.dirs("testsim", recursive = TRUE), projdirs1)
  expect_error(RunFlow(deck1, "testsim", sim_exec = "noexec"),
               "Failed to locate ")
})
# print(list.dirs("testsim", recursive = TRUE))
# clean up
unlink("testsim", recursive = TRUE)
